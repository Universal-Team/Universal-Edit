...
void Input::DrawNumpad() {
	for(uint i = 0; i < (sizeof(NumpadStruct) / sizeof(NumpadStruct[0])); i++) {
		C2D_DrawRectSolid(NumpadStruct[i].x, NumpadStruct[i].y, 0.5f, 60, 50, Config::Color2 & C2D_Color32(255, 255, 255, 200));
...
std::string Input::getLine(uint maxLength, std::string Text) {
	...
	while(1) {
		...
		if (hDown & KEY_START || enter) break;
	}
...
int Input::getUint(int max, std::string Text) {
	const std::string s = Input::Numpad(3, Text);
...
char Input::checkKeyboard(int hDown, int hHeld) {
	...
	if (caps) C2D_DrawRectSolid(modifierKeys[1].x, modifierKeys[1].y + (103), 0.5f, 20, 20, Config::Color1 & C2D_Color32(200, 200, 200, 200));
	if (shift) C2D_DrawRectSolid(modifierKeys[2 + shift].x, modifierKeys[2 + shift].y + (103), 0.5f, modifierKeys[2 + shift].w, 20, Config::Color1 & C2D_Color32(200, 200, 200, 200));
...
std::string Input::Numpad(uint maxLength, std::string Text) {
	...
	while(1) {
		...
		if (hDown & KEY_B || touching(touch, Numbers[11])) string = string.substr(0, string.length()-1);

		if (hDown & KEY_START || touching(touch, Numbers[10]) || enter) break;
	}
